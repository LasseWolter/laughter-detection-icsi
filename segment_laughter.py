...
def load_and_pred(audio_path):
    ...
    for setting, instances in instance_dict.items():
        print(
            f"Found {len(instances)} laughs for threshold {setting[0]} and min_length {setting[1]}."
        )
        instance_output_dir = os.path.join(
            output_dir, f"t_{setting[0]}", f"l_{setting[1]}"
        )
        save_instances(
            instances, instance_output_dir, save_to_audio_files, save_to_textgrid
        )
...
def save_instances(instances, output_dir, save_to_audio_files, save_to_textgrid):
    ...
    if len(instances) > 0:
        ...
        if save_to_textgrid:
            ...
            tgt.write_to_file(tg, os.path.join(output_dir, fname + ".TextGrid"))
