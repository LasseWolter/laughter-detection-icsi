...
def get_random_non_laughter_segment(duration, meeting_id, silence=False):
    ...
    speech_seg = P.closed(utils.to_frames(start), utils.to_frames(start + duration))

    if silence and not prep.silence_index[meeting_id][sample_seg.part_id].contains(
        speech_seg
    ):
        return get_random_non_laughter_segment(duration, meeting_id, silence=True)
    elif utils.seg_overlaps(
        speech_seg,
        [prep.laugh_index, prep.invalid_index],
        sample_seg.meeting_id,
        sample_seg.part_id,
    ):
    ...
    # sanity check
    if silence:
        assert prep.silence_index[meeting_id][sample_seg.part_id].contains(
            speech_seg
        ), "Randomly selected silence segment not contained in silence index."
    return [
        start,
        duration,
        sub_start,
...
def get_random_segment_from_df(duration, meeting_id, df):
    ...
    sub_start, sub_duration = get_subsample(
        row.start, row.length, cfg["train"]["subsample_duration"]
    )
...
def get_subsample(start, duration, subsample_duration):
    ...
    subsample_start = np.random.uniform(start, start + duration - sub_dur)
    return subsample_start, sub_dur


def create_data_df(
    data_dir,
    num_of_laugh_samples,
    num_of_non_laugh_samples,
    meeting_id=None,
    chan_id=None,
    random=False,
):
    ...
    if meeting_id:
        ...
        if chan_id:
            subset_of_laughs = df[
                (df.meeting_id == meeting_id) & (df.chan_id == chan_id)
            ]
        else:
    ...
    for meeting_id, meeting_laugh_df in tqdm(
        meeting_groups, desc="Sampling segments for each meeting"
    ):
        split = "train"
        ...
        for _, laugh_seg in meeting_laugh_df.iterrows():
            ...
            if random:
                ...
                for _ in range(0, speech_segs):
                    non_laugh_seg_list[split].append(
                        get_random_segment_from_df(
                            laugh_seg.length, meeting_id, df=parse.speech_df
                        )
                    )
                ...
                for _ in range(0, silence_segs):
                    non_laugh_seg_list[split].append(
                        get_random_non_laughter_segment(
                            laugh_seg.length, meeting_id, silence=True
                        )
                    )

            # Subsample laugh segment and append to list
            audio_path = os.path.join(laugh_seg.meeting_id, f"{laugh_seg.chan_id}.sph")

            for i in range(num_of_laugh_samples):
                sub_start, sub_duration = get_subsample(
                    laugh_seg.start,
                    laugh_seg.length,
                    cfg["train"]["subsample_duration"],
                )
    ...
    for split in PARTITIONS.keys():  # [train,,test]
        ...
        assert (
            whole_df[whole_df.start < 0].shape[0] == 0
        ), "Found row with negative start-time"
        assert (
            whole_df[whole_df.duration < 0].shape[0] == 0
        ), "Found row with negative duration"
        assert (
            whole_df[whole_df.sub_start < 0].shape[0] == 0
        ), "Found row with negative sub_start-time"
        assert (
            whole_df[whole_df.sub_duration < 0].shape[0] == 0
        ), "Found row with negative sub_duration"

        # Check that only valid lables are in the dataframe
        assert (
            whole_df[~whole_df.label.isin([0, 1])].shape[0] == 0
        ), "There are labels which are not 0 or 1"
        ...
        assert (
            len(mismatched_meetings) == 0
        ), f"Found meetings in {split}_df with meeting_id not corresponding to that split, namely: {mismatched_meetings}"
...
if __name__ == "__main__":
    ...
    create_data_df(
        ...
        num_of_laugh_samples,
        num_of_non_laugh_samples,
        random=random_segment_selection,
    )
