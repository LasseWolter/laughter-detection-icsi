...
def create_manifest(
    audio_dir, transcripts_dir, output_dir, force_manifest_reload=False
):
    """
    ...
    if os.path.isdir(output_dir) and not force_manifest_reload:
        ...
        for split in SPLITS:
            icsi[split] = {}
            rec_set = RecordingSet.from_jsonl(
                os.path.join(output_dir, f"recordings_{split}.jsonl")
            )
...
def compute_features_per_split(
    icsi_manifest, output_dir, num_jobs=8, use_kaldi=False, force_recompute=False
):
    """
    ...
    extractor = get_feat_extractor(
        num_samples=cfg.FEAT["num_samples"],
        num_filters=cfg.FEAT["num_filters"],
        use_kaldi=use_kaldi,
    )
    ...
    for split in SPLITS:
        ...
        split_cutset = CutSet.from_manifests(
            recordings=icsi_manifest[split]["recordings"]
        )
        ...
        if use_kaldi:
            ...
            split_feat_cuts = split_cutset.compute_and_store_features_batch(
                extractor=extractor, storage_path=feats_dir, num_workers=num_jobs
            )
        else:
            split_feat_cuts = split_cutset.compute_and_store_features(
                extractor=extractor, storage_path=feats_dir, num_jobs=num_jobs
            )

        split_feat_cuts.to_jsonl(os.path.join(cutset_dir, f"{split}_feats.jsonl"))


def compute_features_for_cuts(
    icsi_manifest,
    data_dfs_dir,
    output_dir,
    split_feats_dir,
    ...
    split_feat_cutsets = {}
    for split in SPLITS:
        dfs[split] = pd.read_csv(os.path.join(data_dfs_dir, f"{split}_df.csv"))

        split_feat_cutsets[split] = CutSet.from_jsonl(
            os.path.join(split_feats_dir, "cutsets", f"{split}_feats.jsonl")
        )
    ...
    for split, df in dfs.items():
        ...
        for ind, row in tqdm(
            df.iterrows(),
            total=df.shape[0],
            desc=f"Creating features for {split}-split:",
        ):
            ...
            row_track = split_feat_cutsets[split].filter(
                lambda c: (c.id.startswith(meeting_id) and c.channel == chan_id)
            )[0]

            # Get a cut that represents the subsample from this track
            row_cut = row_track.truncate(
                offset=row.sub_start, duration=row.sub_duration
            ).pad(duration=min_seg_duration, preserve_id=True)

            sup = SupervisionSegment(
                id=f"sup_{row_cut.id}",
                recording_id=row_cut.recording.id,
                start=row.sub_start,
                duration=min_seg_duration,
                channel=chan_id,
                custom={"is_laugh": row.label},
            )
...
def compute_features_for_single_audio_track(
    output_dir,
    split_feats_dir,
    meeting_id,
    channel,
    ...
    split = split
    whole_split_cutset = CutSet.from_jsonl(
        os.path.join(split_feats_dir, "cutsets", f"{split}_feats.jsonl")
    )
    ...
    # Get track for this particular channel in this meeting
    # [0] because we know that only one meeting will match this query
    track_cutsset = whole_split_cutset.filter(
        lambda c: (c.id.startswith(meeting_id) and c.channel == chan_id)
    )

    # Get a cut that represents the subsample from this track
    track_cuts = track_cutsset.cut_into_windows(duration=min_seg_duration).pad(
        duration=min_seg_duration, preserve_id=True
    )
    cut_list = []
    for id, cut in enumerate(track_cuts):
        ...
        if interval.overlaps(prep.laugh_index[meeting_id][part_id]):
            label = 1
        else:
            label = 0
        sup = SupervisionSegment(
    ...
    cuts_with_feats_file_train = os.path.join(
        cutset_dir, f"train_cutset_with_feats.jsonl"
    )
...
def main(env_file=".env"):
    ...
    min_seg_duration = (
        float(os.getenv("MIN_SEG_DURATION")) if os.getenv("MIN_SEG_DURATION") else 1.0
    )
    ...
    compute_features_per_split(
        icsi_manifest=icsi_manifest,
