...
def plot_train_metrics(df_dir, name="metrics_plot", sub_dir="", show=False):
    ...
    if os.path.isfile(os.path.join(df_dir, "train_params.csv")):
        ...
        print(
            "Couldn't load num_train_samples per epoch. Not displaying secondary axis with epochs."
        )
    ...
    for split, plt_rep in splits:
        axs[0, 0].plot(
            df["batch_num"], df[f"{split}_prec"], plt_rep, label=f"{split}_precision"
        )
        axs[0, 1].plot(
            df["batch_num"], df[f"{split}_rec"], plt_rep, label=f"{split}_recall"
        )
        axs[1, 0].plot(
            df["batch_num"], df[f"{split}_acc"], plt_rep, label=f"{split}_accuracy"
        )
        axs[1, 1].plot(
            df["batch_num"], df[f"{split}_loss"], plt_rep, label=f"{split}_loss"
        )
    ...
    out_file = os.path.join(
        cfg.ANALYSIS["plots_dir"], sub_dir, "train_metrics", f"{name}.png"
    )
...
def plot_prec_recall_curve(
    dfs_with_labels, out_name="prec_recall_curve.png", sub_dir="", split="", show=False
):
    """
    ...
    cols = ["b", "g", "r", "c", "k", "y", "p"]
    if len(dfs_with_labels) > len(cols):
        print("More plots than colours, colours will be repeated")
    ...
    out_file = os.path.join(
        cfg.ANALYSIS["plots_dir"], sub_dir, "prec_recall", f"{split}_{out_name}"
    )
    Path(out_file).parent.mkdir(exist_ok=True, parents=True)
    plt.savefig(out_file)
    if show:
...
def plot_conf_matrix(
    ...
    sum_vals = (
        eval_df.groupby(["threshold", "min_len"])[
            [
                "corr_pred_time",
                "tot_pred_time",
    ...
    hm = sns.heatmap(
        conf_ratio,
        yticklabels=sum_vals["threshold"],
    ...
    plot_file = os.path.join(
        cfg.ANALYSIS["plots_dir"], sub_dir, "conf_matrix", f"{name}.png"
    )
...
def compare_prec_recall(
    dirs_with_labels, min_len, thresholds, split, sub_dir="", show=False
):
    """
    ...
    plot_prec_recall_curve(
        dfs,
        out_name="compare_class_balance_dev_set",
        sub_dir=sub_dir,
        split=split,
        show=show,
    )
...
def main():
    ...
    visualise_experiment(
        dirs, labels, "exp_1", conf_thrs=four_thrs, prec_rec_thrs=all_thrs
    )
    ...
    labels = ["baseline-1-to-20", "model-a", "model-b", "model-c"]  # used as baseline
    visualise_experiment(
        dirs, labels, "exp_2", conf_thrs=four_thrs, prec_rec_thrs=all_thrs
    )
