...
def random_augment(y, sr, noise_signals, impulse_responses):
    '''
    Apply augmentation by applying the following functions (in random order):
        - random_noise 
        - random_reverb
    '''
...
def random_augment_strong(y, sr, noise_signals, impulse_responses):
    '''
    Apply stronger augmentation than in random_augment applying more functions (in random order):
        - random_stretch
        - random_speed
        - random_pitch
        - random_noise
...
def set_length(new_y, y):
    ''' 
    Used by random_speed to keep length of a sample when speeding up/slowing down. 
    factor = y/new_y
    If factor is >1 (new_y < y), we've sped up, so we need to pad. 
    If factor <1 (new_y > y), take the beginning of y set length of new_y to match length of y
    '''
    if len(new_y) < len(y):
...
def random_noise(y, sr, noise_signals, min_snr=6, max_snr=30, prob=1.):
    if np.random.uniform(0, 1) < prob:
        ...
        if len(noise_signal) < len(y):
            raise Exception(
                "length of the background noise signal is too short")
        ...
        loudness_normalized_noise = pyln.normalize.loudness(
            noise, noise_loudness, sig_loudness-snr)
...
def subsample_time(offset, duration, audio_file_length, subsample_length=1., padding_length=0.5, random_seed=None):
    ...
    start = np.maximum(start_time, np.random.uniform(
        start_time, end_time-subsample_length))
