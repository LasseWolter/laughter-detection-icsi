...
def create_embedding_layer(weight_matrix, trainable=True):
    ...
    embedding_layer.load_state_dict(
        {'weight': torch.from_numpy(weight_matrix)})
    if not trainable:
...
def run_epoch(model, mode, device, loss_type='x_ent', optimizer=None, clip=1,
    ...
    def _get_ctc_loss(output, trg, device):
        ...
        for i in range(ctc_batch_size):
            ...
            if len(nz) == 0 or nz[0] > ctc_input_length/2:
                ctc_trg_list.append(torch.tensor(
                    [int(ctc_input_length/2)]).to(device))
    ...
    def _eval_for_logging(model, device, val_itr, val_iterator,
        ...
        for j in range(val_batches_per_log):
            try:
                ...
                val_losses.append(_eval_batch(
                    model, device, val_batch, loss_type, teacher_forcing_ratio=0))
    ...
    def _eval_batch(model, device, batch, loss_type, teacher_forcing_ratio=0.,
        ...
        with torch.no_grad():
            ...
            if loss_type == 'ctc':
                ...
                # output_vocab[text_utils.PAD_SYMBOL]
    ...
    if iterator is not None:
        ...
        for i, batch in tqdm(enumerate(iterator)):
            batch_loss = _run_batch(
                model, device, batch, loss_type, teacher_forcing_ratio, batch_index=i)
            ...
            if log_frequency is not None and (model.global_step + 1) % log_frequency == 0:
                ...
                writer.add_scalar(
                    'train/loss', train_loss_at_step, model.global_step)
                writer.add_scalar(
                    'eval/loss', val_loss_at_step, model.global_step)
        ...
        for batch_index in range(len(batches)):
            epoch_loss += _run_batch(model, device,
                                     batches[batch_index], loss_type, teacher_forcing_ratio)
