...
def run_epoch(
    ...
    def _eval_batch(
        model, device, batch, batch_index=None, clip=None, return_raw=False
    ):
        """
        Evaluates one batch
    ...
    if mode.lower() == "train" and validate_online:
        ...
        validations__per_epoch = iterator.sampler.num_cuts / (
            batch_size * log_frequency
        )
        val_batches_per_log = int(
            val_iterator.sampler.num_cuts / validations__per_epoch
        )
    ...
    if iterator is not None:
        ...
        for i, batch in tqdm(enumerate(iterator)):
            ...
            lr = (learning_rate - min_learning_rate) * decay_rate ** (
                float(model.global_step)
            ) + min_learning_rate
            optimizer.lr = lr

            batch_loss, batch_acc, batch_prec, batch_recall = _run_batch(
                model, device, batch, batch_index=i, clip=clip
            )
            ...
            if (
                ...
                and (model.global_step + 1) % log_frequency == 0
            ):
                ...
                (
                    ...
                    model, device, val_itr, val_iterator, val_batches_per_log
                )
            ...
            if (
                ...
                and (model.global_step + 1) % checkpoint_frequency == 0
            ):
                state = torch_utils.make_state_dict(
                    model,
                    optimizer,
                    model.epoch,
                    model.global_step,
...
if os.path.exists(checkpoint_dir) and os.path.isfile(
    os.path.join(checkpoint_dir, "last.pth.tar")
):
...
def get_audios_from_text_data(data_file_or_lines, h, sr=sample_rate):
    ...
    for i in tqdm(range(len(audio_paths))):
        aud = h[audio_paths[i]][
            int(offsets[i] * sr) : int((offsets[i] + durations[i]) * sr)
        ]
        audios.append(aud)
    return audios
...
def update_metrics_on_disk():
    ...
    for batch_num, entry_dict in METRICS_DICT.items():
        ...
        metric_rows.append(
            [batch_num, entry_dict["train"].epoch] + train_entry + val_entry
        )
