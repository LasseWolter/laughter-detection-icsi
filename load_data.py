...
def create_training_dataloader(cutset_dir, split, shuffle=False):
    ...
    cuts = CutSet.from_jsonl(
        os.path.join(cutset_dir, f"{split}_cutset_with_feats.jsonl")
    )
...
def create_inference_dataloader(audio_path):
    ...
    cut = MonoCut(
        id="inference-cut", start=0.0, duration=rec.duration, channel=0, recording=rec
    )

    extractor = get_feat_extractor(
        num_samples=cfg.FEAT["num_samples"], num_filters=cfg.FEAT["num_filters"]
    )
